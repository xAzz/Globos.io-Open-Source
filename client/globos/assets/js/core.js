...
function cargarServers() {
    ...
    var loadGameservers = function(response) {
        ...
        $.each(response.gameservers, function(region, modes) {
            var regiondiv = "<div class=\"server-list\">";
            $.each(modes, function(mode, instances) {
                $.each(instances, function(id, websocket) {
                    var instancediv = "";
                    instancediv += "<div class='server' id=\"" + mode + "\" onclick=\"connector('" + websocket + "', '" + region + "', '" + mode + "', '" + id + "');return false;\">";
                    instancediv += mode + "<span class=\"server-name\"> #" + id + " </span>";
                    instancediv += `<span id="svPlCount" class=\"server-status-players status-${mode}-${id}\" style="float: right;"></span>`;
                    instancediv += "</div>";
